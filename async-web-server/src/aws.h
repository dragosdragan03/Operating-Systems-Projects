...
struct connection {
	...
	int eventfd; // file descriptor pentru o notificare; asigura comunicarea intre procese
	int sockfd; // filedescriptorul socketului care leaga serverul de client

	io_context_t ctx;
	struct iocb iocb; // control block pentru un singur event
	struct iocb *piocb[1];
	size_t file_size;
	...
	size_t send_len; // lungimea datei pe care vrei sa o trimiti
	size_t send_pos; // pozitia in bufferul pe care vrei sa l trimit
	size_t file_pos; // pozitia in fisier
	size_t async_read_len; // lungimea datei citita asincron
